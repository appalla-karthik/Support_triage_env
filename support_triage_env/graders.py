...
class BaseTaskGrader(ABC):
    ...
    def _expectation_where(self, predicate) -> TicketExpectation | None:
...
class MixedQueueCommandCenterGrader(BaseTaskGrader):
    def grade(self, state: SupportTriageState) -> GradingSnapshot:
        security_expectation = self._expectation_where(
            lambda expectation: expectation.category
            in {
                TicketCategory.SECURITY_ACCOUNT_TAKEOVER,
                TicketCategory.SECURITY_ESCALATION,
            }
            or expectation.team == TicketTeam.TRUST_SAFETY
        )
        outage_expectation = self._expectation_where(
            lambda expectation: expectation.category
            in {
                TicketCategory.PRODUCT_BUG,
                TicketCategory.INCIDENT_COORDINATION,
            }
            or expectation.team == TicketTeam.ENGINEERING
        )
        refund_expectation = self._expectation_where(
            lambda expectation: expectation.category
            in {
                TicketCategory.BILLING_REFUND,
                TicketCategory.BILLING_APPROVAL,
            }
            or expectation.team == TicketTeam.BILLING_OPS
        )

        if not (security_expectation and outage_expectation and refund_expectation):
            return self._finalize_grade(
                state,
                {},
                {"scenario_configuration_missing": 1.0},
                [],
                ["Mixed queue expectations are incomplete for grading"],
                ["Scenario configuration missing one or more expected queue archetypes"],
            )
