...
def _project_docs_build_available() -> bool:
    return PROJECT_DOCS_BUILD_DIR.exists()
  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      body.sidebar-expanded .rail-link .rail-code.compact-only,
      body.sidebar-expanded .rail-brand .rail-code.compact-only,
      body.sidebar-expanded .rail-avatar .rail-code.compact-only {
        display: inline;
        font-size: 12px;
        font-weight: 800;
      ...
      body[data-theme="light"] .rail-brand {
        background: linear-gradient(180deg, #7a6cff, #5f74ff);
        color: #ffffff;
      }
      body[data-theme="light"] .rail-brand-text,
      body[data-theme="light"] .rail-code.compact-only,
      body[data-theme="light"] .rail-toggle {
        color: #44506a;
      }
      ...
      @media (max-width: 720px) {
        ...
        .rail-brand .rail-code.compact-only,
        body.sidebar-expanded .rail-brand .rail-code.compact-only {
          font-size: 14px;
        }
        .rail-link,
        ...
        .rail-link .rail-code.compact-only,
        body.sidebar-expanded .rail-link .rail-code.compact-only {
          font-size: 12px;
          font-weight: 800;
          letter-spacing: 0.04em;
        ...
        body.mobile-nav-open .rail-menu {
        ...
        body[data-theme="light"] .rail-menu {
          background: rgba(255, 255, 255, 0.96);
          border-color: rgba(73, 95, 135, 0.16);
          box-shadow: 0 18px 36px rgba(50, 63, 96, 0.16);
        }
        body[data-theme="light"] .rail-menu {
          border-right: 1px solid rgba(73, 95, 135, 0.14);
        }
        body[data-theme="light"] .mobile-nav-backdrop {
          background: rgba(73, 95, 135, 0.22);
        }
        body[data-theme="light"] .rail-toggle,
        body[data-theme="light"] .rail-link {
          background: rgba(111, 89, 255, 0.08);
        ...
        body[data-theme="light"] .rail-link:hover {
          background: linear-gradient(180deg, rgba(111, 89, 255, 0.94), rgba(135, 93, 255, 0.9));
          border-color: rgba(111, 89, 255, 0.28);
          color: #ffffff;
        }
        body[data-theme="light"] .rail-toggle:hover {
          background: rgba(111, 89, 255, 0.14);
          border-color: rgba(111, 89, 255, 0.22);
          color: #263248;
        }
        .rail-quick-code,
        .quick-code,
      ...
      <div class="dashboard-shell">
        <aside class="nav-rail" aria-label="Dashboard navigation">
          <div class="rail-top">
            ...
            <a class="rail-brand" href="/dashboard#top" data-route="/dashboard#top" data-target="top" title="TriageOS Home">
              <span class="rail-code compact-only">TS</span>
              <span class="rail-glyph">TS</span>
              <span class="rail-brand-text">TriageOS</span>
            </a>
            <button class="rail-toggle" id="rail-toggle" type="button" aria-expanded="false" aria-controls="rail-menu" aria-label="Open navigation menu">
              <span class="rail-code compact-only">≡</span>
              <span class="rail-glyph">≡</span>
            </button>
            </div>
            <div class="rail-menu" id="rail-menu">
            <a class="rail-link active" href="/dashboard#top" data-route="/dashboard#top" data-target="top" title="Dashboard">
              <span class="rail-code compact-only">DB</span>
              <span class="rail-glyph">DB</span>
              <span class="rail-label">Dashboard</span>
            </a>
            <a class="rail-link" href="/operations#tickets" data-route="/operations#tickets" data-target="tickets" title="Operations">
              <span class="rail-code compact-only">OP</span>
              <span class="rail-glyph">OP</span>
              <span class="rail-label">Operations</span>
            </a>
            <a class="rail-link" href="/inspectors#analytics" data-route="/inspectors#analytics" data-target="analytics" title="Inspectors">
              <span class="rail-code compact-only">IN</span>
              <span class="rail-glyph">IN</span>
              <span class="rail-label">Inspectors</span>
            </a>
            <a class="rail-link" href="/api" data-route="/api" title="API Docs">
...
GET  /schema</div>
    ...
    <script>
      ...
      const dashboardPageTargets = {
        "/": "top",
        "/dashboard": "top",
        "/operations": "tickets",
        "/inspectors": "analytics",
      };
      const railBrandLink = document.querySelector(".nav-rail .rail-brand");
      const railNavLinks = Array.from(document.querySelectorAll(".nav-rail .rail-link"));
      const railNavActions = [railBrandLink, ...railNavLinks].filter(Boolean);
      let mobileNavLastHref = "";
      let mobileNavLastAt = 0;
      const railToggleCode = railToggle ? railToggle.querySelector(".rail-code.compact-only") : null;
      const railToggleGlyph = railToggle ? railToggle.querySelector(".rail-glyph") : null;
      const setRailToggleIcon = (isOpen) => {
        const icon = isOpen ? "\u2715" : "\u2261";
        const label = isOpen ? "Close navigation menu" : "Open navigation menu";
        if (railToggleCode) {
          railToggleCode.textContent = icon;
        }
        if (railToggleGlyph) {
          railToggleGlyph.textContent = icon;
        }
        if (railToggle) {
          railToggle.setAttribute("aria-label", label);
        }
        if (railMenu) {
          railMenu.setAttribute("aria-hidden", String(!isOpen && mobileNavMedia.matches));
        }
      };
      if (themeToggle) {
      ...
      const syncResponsiveNav = () => {
        if (mobileNavMedia.matches) {
          ...
          if (railToggle) {
            railToggle.setAttribute("aria-expanded", String(document.body.classList.contains("mobile-nav-open")));
          }
          if (railMenu) {
            railMenu.setAttribute("aria-hidden", String(!document.body.classList.contains("mobile-nav-open")));
          }
          setRailToggleIcon(document.body.classList.contains("mobile-nav-open"));
          if (mobileNavBackdrop) {
            mobileNavBackdrop.hidden = !document.body.classList.contains("mobile-nav-open");
          }
          return;
        }
        document.body.classList.remove("mobile-nav-open");
        setRailToggleIcon(false);
        if (railMenu) {
          railMenu.setAttribute("aria-hidden", "false");
        }
        if (mobileNavBackdrop) {
          mobileNavBackdrop.hidden = true;
        }
        const nextSidebarState = window.localStorage.getItem("triage-sidebar-expanded") === "true";
        document.body.classList.toggle("sidebar-expanded", nextSidebarState);
        if (railToggle) {
          railToggle.setAttribute("aria-expanded", String(document.body.classList.contains("sidebar-expanded")));
        }
      };
      syncResponsiveNav();
      if (typeof mobileNavMedia.addEventListener === "function") {
        mobileNavMedia.addEventListener("change", syncResponsiveNav);
      } else if (typeof mobileNavMedia.addListener === "function") {
        mobileNavMedia.addListener(syncResponsiveNav);
      }
      ...
      const setActiveRailLink = (activeLink) => {
        railNavLinks.forEach((link) => link.classList.toggle("active", link === activeLink));
      };
      const runDashboardNavigation = (targetId, pushHash = true) => {
        if (!targetId) {
          return false;
        }
        if (mobileNavMedia.matches) {
          closeMobileNav();
          window.setTimeout(() => {
            scrollToDashboardTarget(targetId, pushHash);
          }, 190);
          return true;
        }
        return scrollToDashboardTarget(targetId, pushHash);
      };
      const scrollToDashboardTarget = (targetId, pushHash = true) => {
        if (!targetId) {
          return false;
        }
        ...
        const viewportOffset = mobileNavMedia.matches ? 88 : 32;
        ...
        if (pushHash && window.location.pathname === "/") {
          window.history.replaceState(null, "", `/#${targetId}`);
        }
        return true;
      };
      const activateRailNavLink = (link, event) => {
        const href = link.getAttribute("href");
        if (mobileNavMedia.matches && href) {
          event.preventDefault();
          event.stopPropagation();
          const now = Date.now();
          if (mobileNavLastHref === href && now - mobileNavLastAt < 700) {
            return;
          }
          mobileNavLastHref = href;
          mobileNavLastAt = now;
      ...
      railNavActions.forEach((link) => {
        link.addEventListener("pointerup", (event) => {
          activateRailNavLink(link, event);
        });
      ...
      document.addEventListener("keydown", (event) => {
        if (event.key === "Escape" && mobileNavMedia.matches && document.body.classList.contains("mobile-nav-open")) {
          closeMobileNav();
          if (railToggle) {
            railToggle.focus();
          }
      ...
      menuButtons.forEach((button) => {
        button.addEventListener("click", () => {
          ...
          runDashboardNavigation(targetId);
          menuButtons.forEach((item) => item.classList.remove("active"));
          button.classList.add("active");
          const matchingRailLink = railNavLinks.find((link) => (link.getAttribute("href") || "").endsWith(`#${targetId}`));
          if (matchingRailLink) {
            setActiveRailLink(matchingRailLink);
          }
          if (mobileNavMedia.matches && !targetId) {
            closeMobileNav();
          }
        });
      });
      const syncHashNavigation = () => {
        ...
        const matchingRailLink = railNavLinks.find((link) => link.getAttribute("data-target") === targetId);
        if (matchingRailLink) {
          setActiveRailLink(matchingRailLink);
        }
        scrollToDashboardTarget(targetId, false);
      };
      const syncPathNavigation = () => {
        const currentPath = normalizeDashboardPath(window.location.pathname);
        const targetId = dashboardPageTargets[currentPath];
        const matchingRailLink = railNavLinks.find((link) => {
          const route = link.getAttribute("data-route");
          return normalizeDashboardPath(route || "") === currentPath;
        });
        if (matchingRailLink) {
          setActiveRailLink(matchingRailLink);
        }
...
@app.get("/inspectors", response_class=HTMLResponse, include_in_schema=False)
async def inspectors_page() -> str:
    return await root()


@app.get("/project-docs", response_class=HTMLResponse, include_in_schema=False)
async def project_docs() -> HTMLResponse:
    if not _project_docs_build_available():
        return HTMLResponse(_render_docs_home())
    return HTMLResponse(
        '<!DOCTYPE html><html><head><title>Docs Build Required</title></head><body style="font-family:Segoe UI,sans-serif;padding:40px;"><h1>Project docs are not built yet.</h1><p>Build the local <code>documentation_openenv_customer.support</code> site to serve exact docs here.</p></body></html>'
    )
...
@app.get("/project-docs/", response_class=HTMLResponse, include_in_schema=False)
async def project_docs_trailing_slash() -> HTMLResponse:
    if not _project_docs_build_available():
        return HTMLResponse(_render_docs_home())
    return HTMLResponse(
        '<!DOCTYPE html><html><head><title>Docs Build Required</title></head><body style="font-family:Segoe UI,sans-serif;padding:40px;"><h1>Project docs are not built yet.</h1><p>Build the local <code>documentation_openenv_customer.support</code> site to serve exact docs here.</p></body></html>',
...
@app.get("/project-docs/docs/{slug}", response_class=HTMLResponse, include_in_schema=False)
async def project_docs_rendered_page(slug: str) -> HTMLResponse:
    if not _project_docs_build_available():
        if slug not in DOCS_META:
            return HTMLResponse("<h1>Document not found.</h1>", status_code=404)
        return HTMLResponse(_render_docs_page(slug))
    return HTMLResponse(
        '<!DOCTYPE html><html><head><title>Docs Build Required</title></head><body style="font-family:Segoe UI,sans-serif;padding:40px;"><h1>Project docs are not built yet.</h1><p>Build the local <code>documentation_openenv_customer.support</code> site to serve exact docs pages here.</p></body></html>',
        status_code=404,
    )
